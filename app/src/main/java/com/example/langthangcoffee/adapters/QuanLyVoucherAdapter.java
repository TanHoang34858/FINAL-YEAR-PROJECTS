...
public class QuanLyVoucherAdapter extends RecyclerView.Adapter<QuanLyVoucherAdapter.FoodOrderViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final FoodOrderViewHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        holder.tvSoDienThoai.setText("Phone number: " + voucher.getSdtTaiKhoan());
        if (voucher.getSuDung() == 1) {
            holder.tvTinhTrangSuDung.setText("Used");
        } else {
            holder.tvTinhTrangSuDung.setText("Not yet used");
        ...
        try {
            ...
            if (difference <=0) {
                thoiGianHetHan = "Expiredn";
            } else {
                thoiGianHetHan = "Expires in " + days + " day " +  hours + " hour " + minutes + " minute";
            }
